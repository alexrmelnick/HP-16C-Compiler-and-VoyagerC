...
def parse_interactive(calculator_state):
    ...
    print(f"Output mode determined: {output_mode}")
    ...
    while True:
        try:
            sign_mode = int(input("Sign mode (0 = Unsigned, 1 = 1's complement, 2 = 2's complement, 3 = floating point): "))
            if 0 <= sign_mode <= 3:
...
def parse_cli(argv, calculator_state):
    ...
    if len(argv) == 3:
        ...
        print("python Jovial_Assembler.py <input file (.jov)> <output file (.16c/.pdf)> [assumes  2's complement, 16-bit, decimal]")
        print("python Jovial_Assembler.py <input file (.jov)> <output file (.16c/.pdf)> <sign mode (0/1/2/3)> <word size (4-64)> <base (2/8/10/16)>")
        sys.exit(1)
